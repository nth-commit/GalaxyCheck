...
namespace GalaxyCheck.Runners.Check.AutomataAsync
{
    internal record ReplayState<T>(string ReplayEncoded) : CheckState<T>
    {
        ...
        private static async Task<CheckStateTransition<T>> HandleInstance(CheckStateContext<T> ctx, IGenInstance<TestAsync<T>> instance, Replay replayDecoded)
        {
        ...
        private static CheckStateTransition<T> HandleReplayedCounterexample(
            ...
            IGenInstance<TestAsync<T>> instance,
            Replay replayDecoded,
            ExplorationStage<TestAsync<T>>.Counterexample counterexample)
        {
